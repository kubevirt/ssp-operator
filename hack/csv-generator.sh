...
MANIFESTS_GENERATED_DIR="data/olm-catalog"
CRDS_DIR="data/crd"
...
if ! [ -d $MANIFESTS_GENERATED_DIR ]; then
    MANIFESTS_GENERATED_DIR="${HOME}/data/olm-catalog"
    CRDS_DIR="${HOME}/data/crd"
fi
...
while (( "$#" )); do
    ...
    --validator-image)
        ...
        ;;
    --virt-launcher-image)
        VIRT_LAUNCHER_IMAGE=$VAL
        ;;
    --node-labeller-image)
        NODE_LABELLER_IMAGE=$VAL
        ;;
    --cpu-plugin-image)
        CPU_PLUGIN_IMAGE=$VAL
        ;;
    --dump-crds)
        DUMP_CRDS="true"
        ;;
...
replace_env_var "VIRT_LAUNCHER_IMAGE" $VIRT_LAUNCHER_IMAGE
replace_env_var "NODE_LABELLER_IMAGE" $NODE_LABELLER_IMAGE
replace_env_var "CPU_PLUGIN_IMAGE" $CPU_PLUGIN_IMAGE
