...
func (d *dataSources) getDataSourcesAndCrons(request *common.Request) (dataSourcesAndCrons, error) {
	...
	if isMultiarch {
		var err error
		cronByDataSource, err = getCronsByDataSourceMultiArch(d.sourceCollection, &request.Instance.Spec)
		if err != nil {
			return dataSourcesAndCrons{}, fmt.Errorf("failed to create DataImportCrons: %w", err)
		}
	} else {
...
func getCronsByDataSource(sspSpec *ssp.SSPSpec) map[client.ObjectKey]cdiv1beta1.DataImportCron {
	...
	for i := range cronTemplates {
		cron := cronTemplates[i].AsDataImportCron()
		if cron.Namespace == "" {
...
func getCronsByDataSourceMultiArch(sourceCollection template_bundle.DataSourceCollection, sspSpec *ssp.SSPSpec) (map[client.ObjectKey]cdiv1beta1.DataImportCron, error) {
	...
	for i := range cronTemplates {
		...
		cronArchs := parseArchsAnnotation(archsAnnotationValue)
...
func handleDefaultDataSource(dataSource *cdiv1beta1.DataSource, autoUpdate bool, originalPvcName string, request *common.Request) (*cdiv1beta1.DataSource, bool, error) {
	...
	dsNotFound := errors.IsNotFound(err)
	if dsNotFound {
		err := request.UncachedReader.Get(request.Context, client.ObjectKey{
...
func parseArchsAnnotation(value string) []architecture.Arch {
