...
func getClusterArchitectures(sspObj *ssp.SSP) []string {
	if sspObj.Spec.Cluster == nil {
		if ptr.Deref(sspObj.Spec.EnableMultipleArchitectures, false) {
			// If multiple architectures are enabled, but cluster is nil,
			// we don't know what architecture to use.
			return nil
		}
...
func (c *commonTemplates) deprecateOrDeleteOldTemplates(request *common.Request, deployedTemplates []templatev1.Template, archs []string) ([]common.ReconcileFunc, error) {
