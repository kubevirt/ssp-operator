...
var _ = Describe("Template bundle", func() {
	...
	Context("CollectDataSourceNamesWithArchs", func() {
		It("should collect DataSource names", func() {
			...
			dataSourceNames, err := CollectDataSourceNamesWithArchs(testTemplates)
			Expect(err).ToNot(HaveOccurred())

			Expect(dataSourceNames).To(HaveKeyWithValue(centosDsName, ContainElement(common_templates.TemplateDefaultArchitecture)))
			Expect(dataSourceNames).To(HaveKeyWithValue(windowsDsName, ContainElement(common_templates.TemplateDefaultArchitecture)))
		})
	})
})
