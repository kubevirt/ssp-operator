...
var _ = Describe("DataSources", func() {
	...
	Context("with DataImportCron template", func() {
		...
		Context("without existing PVC and custom namespace", decorators.Conformance, func() {
			var (
		...
		Context("with existing PVC", decorators.Conformance, func() {
			var (
			...
			Context("with CDI label on DataSource", decorators.Conformance, func() {
				BeforeEach(func() {
					Eventually(func(g Gomega) error {
						ds := &cdiv1beta1.DataSource{}
						g.Expect(apiClient.Get(ctx, dataSource.GetKey(), ds)).To(Succeed())
			...
			Context("with custom NameSpace", decorators.Conformance, func() {
				var (
					customNamespace core.Namespace
				)
		...
		Context("with DataImportCron cleanup", decorators.Conformance, func() {
